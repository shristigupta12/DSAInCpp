...
class car{
    ...
    public:
        ...
        void operator = (car &x){   // This will make deep copy work
            price = x.price;
            model = x.model;
            name = new char[strlen(x.name)+1]; // deep copy, otherwise in default there is shallow copy
            strcpy(name, x.name);
...
int main(){
    ...
    // car d(c);   Instead of this way of making copy constructor
    car d;
    d = c;       //If we use this way of making a copy constructor then deep copy is not working
